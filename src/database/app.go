...
import (
	...
	"github.com/marinellirubens/dbwrapper/src/internal/logger"
	"github.com/marinellirubens/dbwrapper/src/internal/utils"
)

// METHOD_NOT_ALLOWED ... message for http response
...
func writeBodyAndReturn(result []byte, status int, w http.ResponseWriter, app *App) {
...
func (app *App) GetQueryFromDatabase(w http.ResponseWriter, r *http.Request) {
	var err error
	var ret []byte
	...
	db, ok := app.DbConns[dbID]
	if !ok {
		ret = []byte("Database not located\n")
		writeBodyAndReturn(ret, http.StatusNotFound, w, app)
		return
	}
	...
	if err != nil {
		ret = []byte(fmt.Sprintf("Error processing request %v", err))
		writeBodyAndReturn(ret, http.StatusInternalServerError, w, app)
		return
	}
	...
	allgeneric := make([]map[string]any, 0)
	colvals := make([]any, len(cols))
	for rows.Next() {
		colassoc := make(map[string]any, len(cols))
		for i := range colvals {
			colvals[i] = new(any)
		}
	...
	ret = []byte(js)
	writeBodyAndReturn(ret, http.StatusOK, w, app)
	return
}
